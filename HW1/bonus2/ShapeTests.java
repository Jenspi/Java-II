...
public class ShapeTests{
	...
	private static Point2D p8;

	// CIRCLE TESTS:
	@BeforeClass
	public static void setupCircle(){
		p8 = new Point2D(1.0, 1.0);
		double diameter = 2.0;
		circle = new Circle(p1, diameter);
	}

	// check to make sure object being created is correct
	@Test
    public void testCircleIntialState(){
        assertEquals(1.0, p8.getX(), 0.0);
        assertEquals(1.0, p8.getY(), 0.0);

        assertEquals(2.0, circle.getDiameter(), 0.0);
    }


    // test getCircumference() circle method
	@Test
	public void getCircleCircumferenceTest(){
		double circumference = circle.getCircumference();
		assertEquals(6.283, circumference, 0.001);
	}

	// test getRadius() circle method
	@Test
	public void getCircleRadiusTest(){
		double radius = circle.getRadius();
		assertEquals(1.0, radius, 0.001);
	}

	// test getDiameter() circle method
	@Test
	public void getCircleDiameterTest(){
		double diameter = circle.getDiameter();
		assertEquals(2.0, diameter, 0.001);
	}

	// test getArea() circle method
	@Test
	public void getCircleAreaTest(){
		double area = circle.getArea();
		assertEquals(3.141, area, 0.001);
	}

	// test setCenter() circle method
	public void setCircleCenterTest(){
		p8 = new Point2D(23.4, 23.4);
		circle.setCenter(p8);
		assertEquals(23.4, p8.getX(), 0.001);
		assertEquals(23.4, p8.getY(), 0.001);
	}

	// test setDiameter() circle method
	public void setCircleDiameterTest(){
		double diameter = 4.0;
		circle.setDiameter(diameter);
		assertEquals(4.0, diameter, 0.0);
	}
